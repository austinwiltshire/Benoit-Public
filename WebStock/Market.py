...
class Quarter(object):
	BalanceSheet = BalanceSheet.QuarterlyBalanceSheet
	CashFlowStatement = CashFlowStatement.QuarterlyCashFlowStatement
	IncomeStatement = IncomeStatement.QuarterlyIncomeStatement
	
class Annual(object):
	BalanceSheet = BalanceSheet.AnnualBalanceSheet
	CashFlowStatement = CashFlowStatement.AnnualCashFlowStatement
	IncomeStatement = IncomeStatement.AnnualIncomeStatement
...
class TimeClosure(object):
	...
	def BalanceSheet(self):
		self.getModule().BalanceSheet.fetch(self.symbol, self.time)
	
	@Lazy
	def CashFlowStatement(self):
		self.getModule().CashFlowStatement.fetch(self.symbol, self.time)
		
	@Lazy
	def IncomeStatement(self):
		self.getModule().IncomeStatement.fetch(self.symbol, self.time)
		
	def getModule(self):
		pass
	
class QuarterClosure(TimeClosure):
	def getModule(self):
		return Quarter
	
class AnnualClosure(TimeClosure):
	def getModule(self):
		return Annual
